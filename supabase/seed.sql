...
-- Organization-scoped canary override so the demo organization can exercise
-- Azure DevOps regardless of which deployment environment this seed is
-- loaded under (`next start`, used by CI's e2e run, resolves to
-- 'production'; `next dev` resolves to 'development' and is already
-- enabled globally). This mirrors the real canary rollout mechanism
-- documented in docs/operations/feature-flags.md and only ever matches the
-- fixture organization id created above, which never exists outside a
-- seeded database.
...
values
  (
    '10000000-0000-0000-0000-000000000001',
    'azure_devops_integration', 'staging', true, 100,
    'product-engineering', 'Gate Azure DevOps integration',
    'Demo organization canary for acceptance testing', '2026-09-11', '2027-08-11'
  ),
  (
    '10000000-0000-0000-0000-000000000001',
    'azure_devops_integration', 'production', true, 100,
    'product-engineering', 'Gate Azure DevOps integration',
    'Demo organization canary for acceptance testing', '2026-09-11', '2027-08-11'
  )
on conflict (organization_id, environment, role_scope, key) do update
set
