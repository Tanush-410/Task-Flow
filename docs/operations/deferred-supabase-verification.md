...
Invitation delivery follows a stage → deliver → finalize lifecycle. Staged and failed tokens cannot be accepted and do not participate in the one-active-invitation constraint. Finalization takes the organization/email lock, revokes the prior active token, and activates the delivered token atomically. A failed resend is discarded without disturbing the prior active invite. If delivery succeeds but finalization fails, the action attempts to mark the staging row failed; if that cleanup also fails, the still-pending token remains non-acceptable and requires support reconciliation before retrying.
